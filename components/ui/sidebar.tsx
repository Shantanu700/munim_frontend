  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          // ease-out-quart, not the registry's ease-linear (all four occurrences in this
          // file). A 200ms linear width change stops dead at the end; on the one layout
          // move a merchant triggers several times a day that reads as a jump rather than
          // a collapse. Same curve as everything else in the app — see DESIGN.md §10.
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-out-quart",
      ...
      >
        <div
          ...
          // DESIGN.md edit: the floating panel takes `shadow-card` (§5's only shadow) and
          // no ring — separation comes from surface colour, not an outline. `rounded-2xl`
          // is left alone because the theme clamps it to the 32px --r-xl ceiling.
          className="flex size-full flex-col bg-sidebar group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:shadow-card"
        >
          {children}
        </div>
      </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-out-quart group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-xl px-3 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-out-quart group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
